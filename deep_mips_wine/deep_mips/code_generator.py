...
class CodeGenerator:
    ...
    def generate(self, graph: ComputationGraph, plan: MemoryPlan, model_def: ModelDef) -> str:
        ...
        self.emit_main()
        self.emit_layers()
        self.emit_runtime_lib()
        self.emit_print_output()
        return "\n".join(self.output)

    def emit_runtime_lib(self):
        self.emit_comment("Runtime Library")
        from runtime_lib import RuntimeLibrary
        lib = RuntimeLibrary()
        self.output.append(lib.generate_all())
        self.emit_blank()
    ...
    def emit_layers(self):
        for nid in self.graph.topological_order:
            ...
            if t == NodeType.FUSED_MATMUL_BIAS_RELU:
                ...
                if node.metadata.get("unroll_factor", 0) > 0:
    ...
    def _emit_matmul_loop(self, node: GraphNode, relu: bool):
        ...
        if self.use_fpu:
            ...
            self.emit(f"    bge   $s3, {inp_sz}, {nid}_fpu_iend")
            self.emit(f"    sll   $t1, $s3, 2")
            self.emit(f"    add   $t1, $t7, $t1")
            self.emit(f"    lwc1  $f1, 0($t1)")
            self.emit(f"    mul   $t2, $s3, {out_sz}")
            self.emit(f"    add   $t2, $t2, $s2")
            self.emit(f"    sll   $t2, $t2, 2")
            self.emit(f"    add   $t2, $t8, $t2")
            self.emit(f"    lwc1  $f2, 0($t2)")
            self.emit(f"    mul.s $f3, $f1, $f2")
            self.emit(f"    add.s $f0, $f0, $f3")
            self.emit(f"    addi  $s3, $s3, 1")
            self.emit(f"    j     {nid}_fpu_inner")
            self.emit(f"{nid}_fpu_iend:")
            if relu:
                self.emit(f"    lwc1  $f1, fpu_zero")
                self.emit(f"    c.lt.s $f0, $f1")
                self.emit(f"    bc1f  {nid}_fpu_relu_skip")
                self.emit(f"    mov.s $f0, $f1")
                self.emit(f"{nid}_fpu_relu_skip:")
    ...
    def _emit_sigmoid_fpu(self, node: GraphNode):
        nid = node.id.replace("-", "_")
        out_sz = node.output_size
        self.emit(f"    li    $t4, 0")
        self.emit(f"{nid}_sig_loop:")
        self.emit(f"    bge   $t4, {out_sz}, {nid}_sig_done")
        self.emit(f"    sll   $t5, $t4, 2")
        self.emit(f"    add   $t5, $s0, $t5")
        self.emit(f"    lwc1  $f12, 0($t5)")
        self.emit(f"    sw    $t4, -4($sp)")
        self.emit(f"    sw    $t5, -8($sp)")
        self.emit(f"    jal   fpu_sigmoid")
        self.emit(f"    lw    $t4, -4($sp)")
        self.emit(f"    lw    $t5, -8($sp)")
        self.emit(f"    swc1  $f0, 0($t5)")
        self.emit(f"    addi  $t4, $t4, 1")
        self.emit(f"    j     {nid}_sig_loop")
        self.emit(f"{nid}_sig_done:")
        self.emit_blank()
    ...
    def _emit_softmax(self, node: GraphNode):
        ...
        self.emit(f"    li    $t4, 0")
        if self.use_fpu:
            self.emit(f"    lwc1  $f2, exp_clamp_neg")
    ...
    def emit_print_output(self):
        self.emit("print_single_result:")
        self.emit("    lwc1  $f0, 0($a0)")
        self.emit("    la    $t0, fpu_half")
        self.emit("    lwc1  $f1, 0($t0)")
        self.emit("    c.lt.s $f0, $f1")
        self.emit("    bc1t  print_zero_char")
        self.emit("    li    $a0, 49")
        self.emit("    j     do_print_char")
        self.emit("print_zero_char:")
        self.emit("    li    $a0, 48")
        self.emit("do_print_char:")
        self.emit("    li    $v0, 11")
        self.emit("    syscall")
        self.emit("    li    $a0, 10")
        self.emit("    li    $v0, 11")
