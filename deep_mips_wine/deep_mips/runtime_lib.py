...
class RuntimeLibrary:
    ...
    def emit_fpu_sigmoid(self):
        self.emit("fpu_sigmoid:")
        self.emit("    # Step 1: negate x")
        self.emit("    neg.s $f13, $f12")
        self.emit("    # Step 2: clamp to avoid overflow")
        self.emit("    lwc1  $f14, exp_clamp_neg")
        self.emit("    c.lt.s $f13, $f14")
        self.emit("    bc1t  sigmoid_return_one")
        self.emit("    lwc1  $f14, exp_clamp_pos")
        self.emit("    c.lt.s $f14, $f13")
        self.emit("    bc1t  sigmoid_return_zero")
        self.emit("    # Step 3: range reduction")
        self.emit("    lwc1  $f14, fpu_inv_ln2")
        self.emit("    mul.s $f15, $f13, $f14")
        self.emit("    cvt.w.s $f16, $f15")
        self.emit("    cvt.s.w $f15, $f16")
        self.emit("    lwc1  $f14, fpu_ln2")
        self.emit("    mul.s $f14, $f15, $f14")
        self.emit("    sub.s $f13, $f13, $f14")
        self.emit("    # Step 4: Taylor series for e^r")
        self.emit("    lwc1  $f0,  exp_c0")
        self.emit("    lwc1  $f14, exp_c1")
        self.emit("    mul.s $f14, $f14, $f13")
        self.emit("    add.s $f0,  $f0,  $f14")
        self.emit("    lwc1  $f14, exp_c2")
        self.emit("    mul.s $f14, $f14, $f13")
        self.emit("    mul.s $f14, $f14, $f13")
        self.emit("    add.s $f0,  $f0,  $f14")
        self.emit("    lwc1  $f14, exp_c3")
        self.emit("    mul.s $f14, $f14, $f13")
        self.emit("    mul.s $f14, $f14, $f13")
        self.emit("    mul.s $f14, $f14, $f13")
        self.emit("    add.s $f0,  $f0,  $f14")
        self.emit("    # Step 5: scale by 2^k")
        self.emit("    mfc1  $t1, $f16")
        self.emit("    beq   $t1, $zero, fpu_sig_scale_done")
        self.emit("    slt   $t2, $t1, $zero")
        self.emit("    beq   $t2, $zero, fpu_sig_scale_pos")
        self.emit("fpu_sig_scale_neg:")
        self.emit("    sub   $t1, $zero, $t1")
        self.emit("    la    $t3, fpu_half")
        self.emit("    lwc1  $f14, 0($t3)")
        self.emit("fpu_sig_neg_loop:")
        self.emit("    beq   $t1, $zero, fpu_sig_scale_done")
        self.emit("    mul.s $f0, $f0, $f14")
        self.emit("    addi  $t1, $t1, -1")
        self.emit("    j     fpu_sig_neg_loop")
        self.emit("fpu_sig_scale_pos:")
        self.emit("    la    $t3, fpu_one")
        self.emit("    lwc1  $f14, 0($t3)")
        self.emit("    add.s $f14, $f14, $f14")
        self.emit("fpu_sig_pos_loop:")
        self.emit("    beq   $t1, $zero, fpu_sig_scale_done")
        self.emit("    mul.s $f0, $f0, $f14")
        self.emit("    addi  $t1, $t1, -1")
        self.emit("    j     fpu_sig_pos_loop")
        self.emit("fpu_sig_scale_done:")
        self.emit("    # Step 6: sigmoid = 1 / (1 + exp_result)")
        self.emit("    lwc1  $f14, fpu_one")
        self.emit("    add.s $f0,  $f0, $f14")
        self.emit("    div.s $f0,  $f14, $f0")
        self.emit("    jr    $ra")
        self.emit("sigmoid_return_one:")
        self.emit("    lwc1  $f0, fpu_one")
        self.emit("    jr    $ra")
        self.emit("sigmoid_return_zero:")
        self.emit("    lwc1  $f0, fpu_zero")
        self.emit("    jr    $ra")
        self.emit("")
